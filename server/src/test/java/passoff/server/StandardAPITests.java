...
public class StandardAPITests {
    ...
    public void staticFiles() {
        ...
        Assertions.assertEquals(HttpURLConnection.HTTP_OK, serverFacade.getStatusCode(), "Server response code was " +
                "not 200 OK");
        Assertions.assertNotNull(htmlFromServer, "Server returned an empty file");
        Assertions.assertTrue(htmlFromServer.contains("CS 240 Chess Server Web API"), "file returned did not contain " +
                "an exact match of text from provided index.html");
    }
    ...
    public void successLogin() {
        ...
        Assertions.assertEquals(existingUser.getUsername(), loginResult.getUsername(), "Response did not give the " +
    ...
    public void successRegister() {
        ...
        Assertions.assertEquals(newUser.getUsername(), registerResult.getUsername(), "Response did not have the same " +
                "username as was registered");
    ...
    public void goodJoin() {
        ...
        Assertions.assertEquals(existingUser.getUsername(), listResult.getGames()[0].getWhiteUsername(), "Username of" +
                " joined player not present in list result");
    ...
    public void uniqueAuthorizationTokens() {
        ...
        Assertions.assertNotEquals(existingAuth, loginOne.getAuthToken(), "Authtoken returned by login matched " +
                "authtoken from prior register");
        Assertions.assertNotEquals(existingAuth, loginTwo.getAuthToken(), "Authtoken returned by login matched " +
                "authtoken from prior register");
        Assertions.assertNotEquals(loginOne.getAuthToken(), loginTwo.getAuthToken(), "Authtoken returned by login " +
                "matched authtoken from prior login");
        ...
        Assertions.assertEquals(existingUser.getUsername(), listResult.getGames()[0].getWhiteUsername(), "incorrect " +
                "username on joined game");
    }
    ...
    private void assertHttpOk(TestResult result) {
        Assertions.assertEquals(HttpURLConnection.HTTP_OK, serverFacade.getStatusCode(), ("Server response code was " +
                "not 200 OK (message: %s)").formatted(result.getMessage()));
        Assertions.assertFalse(result.getMessage() != null && result.getMessage().toLowerCase(Locale.ROOT).contains(
                "error"), "Result returned an error message");
    }
    ...
    private void assertHttpError(TestResult result, int statusCode, String message) {
        Assertions.assertEquals(statusCode, serverFacade.getStatusCode(), ("Server response code was not %d %s " +
                "(message: %s)").formatted(statusCode, message, result.getMessage()));
        Assertions.assertNotNull(result.getMessage(), "Invalid Request didn't return an error message");
        Assertions.assertTrue(result.getMessage().toLowerCase(Locale.ROOT).contains("error"), "Error message didn't " +
